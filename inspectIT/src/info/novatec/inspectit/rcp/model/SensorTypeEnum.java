...
public enum SensorTypeEnum {
	...
	AVERAGE_TIMER("info.novatec.inspectit.agent.sensor.method.averagetimer.AverageTimerSensor",
			InspectITImages.IMG_TIMER),
	...
	CHARTING_MULTI_TIMER("info.novatec.inspectit.agent.sensor.method.averagetimer.ChartingMultiTimer",
			InspectITImages.IMG_TIMER),
	/** The invocation sequence sensor type. */
	INVOCATION_SEQUENCE("info.novatec.inspectit.agent.sensor.method.invocationsequence.InvocationSequenceSensor",
			InspectITImages.IMG_INVOCATION),
	/** The sql sensor type. */
	...
	JDBC_CONNECTION("info.novatec.inspectit.agent.sensor.method.jdbc.ConnectionSensor", InspectITImages.IMG_DATABASE,
			false),
	/** Meta data from the connection. */
	JDBC_CONNECTION_META_DATA("info.novatec.inspectit.agent.sensor.method.jdbc.ConnectionMetaDataSensor",
			InspectITImages.IMG_DATABASE, false),
	/** The jdbc statement sensor type. */
	JDBC_STATEMENT("info.novatec.inspectit.agent.sensor.method.jdbc.StatementSensor", InspectITImages.IMG_DATABASE,
			false),
	/** The jdbc prepared statement sensor type. */
	JDBC_PREPARED_STATEMENT("info.novatec.inspectit.agent.sensor.method.jdbc.PreparedStatementSensor",
			InspectITImages.IMG_DATABASE, false),
	/** The jdbc prepared statement parameter sensor type. */
	JDBC_PREPARED_STATEMENT_PARAMETER(
			"info.novatec.inspectit.agent.sensor.method.jdbc.PreparedStatementParameterSensor",
			InspectITImages.IMG_DATABASE, false),
	/** The exception sensor. */
	EXCEPTION_SENSOR("info.novatec.inspectit.agent.sensor.exception.ExceptionSensor",
			InspectITImages.IMG_EXCEPTION_SENSOR),
	/** The exception sensor overview. */
	EXCEPTION_SENSOR_GROUPED("info.novatec.inspectit.agent.sensor.exception.ExceptionSensorOverview",
			InspectITImages.IMG_EXCEPTION_SENSOR),
	/** The combined metrics sensor type. */
	MARVIN_WORKFLOW("info.novatec.inspectit.agent.sensor.method.marvintimer.MarvinWorkflowSensor",
			InspectITImages.IMG_INVOCATION),
	...
	CHARTING_HTTP_TIMER_SENSOR("info.novatec.inspectit.agent.sensor.method.http.ChartingHttpSensor",
			InspectITImages.IMG_HTTP),
	/** The classloading information sensor type. */
	CLASSLOADING_INFORMATION("info.novatec.inspectit.agent.sensor.platform.ClassLoadingInformation",
			InspectITImages.IMG_CLASS_OVERVIEW),
	...
	MEMORY_INFORMATION("info.novatec.inspectit.agent.sensor.platform.MemoryInformation",
			InspectITImages.IMG_MEMORY_OVERVIEW),
	...
	SYSTEM_INFORMATION("info.novatec.inspectit.agent.sensor.platform.SystemInformation",
			InspectITImages.IMG_SYSTEM_OVERVIEW),
	/** The thread information sensor type. */
	THREAD_INFORMATION("info.novatec.inspectit.agent.sensor.platform.ThreadInformation",
			InspectITImages.IMG_THREADS_OVERVIEW),
	/** The navigation invocation sequence sensor type. */
	NAVIGATION_INVOCATION(
			"info.novatec.inspectit.agent.sensor.method.invocationsequence.NavigationInvocationSequenceSensor",
			InspectITImages.IMG_INVOCATION),
	...
	/** Log4JLogging sensor. */
	LOG4J_LOGGING_DATA("info.novatec.inspectit.agent.sensor.method.logging.Log4JLoggingSensor",
			InspectITImages.IMG_LOGGING_MESSAGE),
	...
	DIAGNOSEIT_RESULTS_SEQUENCES("org.diagnoseit.spike.backNavigation", InspectITImages.IMG_DIAGNOSEIT_SEQUENCE);
	...
	/**
	 * Same as standard constructor but the openable can be specified in
	 * addition.
	 * 
	 * @param fqn
	 *            The full qualified name.
	...
	/**
	 * Returns all elements of the enumeration for the given list of
	 * {@link MethodIdentToSensorType} objects.
	 * 
	...
	public static Set<SensorTypeEnum> getAllOf(Set<MethodIdentToSensorType> methodIdentToSensorTypes) {
		...
		for (MethodIdentToSensorType methodIdentToSensorType : methodIdentToSensorTypes) {
			...
			if (null != sensorTypeEnum) {
				...
				// This might happen if we realize a new sensor type and forget
				// to add it to the
				// sensor type enum. We put this here as a failfast reminder.
				throw new RuntimeException(
						"Lookup for the enum of sensor type " + sensorType.getFullyQualifiedClassName() + " fails");
			}
		}
