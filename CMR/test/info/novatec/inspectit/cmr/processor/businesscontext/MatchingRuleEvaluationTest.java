...
public class MatchingRuleEvaluationTest {
	...
	private final NotEvalStrategy notStrategy = new NotEvalStrategy(null);

	private final AndEvalStrategy andStrategy = new AndEvalStrategy(null);

	private final OrEvalStrategy orStrategy = new OrEvalStrategy(null);

	@InjectMocks
	private final StringMatchingEvalStrategy stringStrategy = new StringMatchingEvalStrategy(null);
	...
	public static void initializeInvocationSequence() {
		...
		httpData.getHttpInfo().setUri(URI);
	...
	private void testHTTPUriMatching() {
		...
		Assert.assertTrue(evaluation.evaluate(strMatchingExpression_positive, iSeqRoot));
		...
		Assert.assertFalse(evaluation.evaluate(strMatchingExpression_negative, iSeqRoot));
	}
